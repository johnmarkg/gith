...
function simplifyBitbucketPayload(payload){
  var branch = '';
  var tag = '';
  ...
  var simpler = {
    ...
    tag: tag,
    branch: branch,
    repo: payload.repository ? (payload.repository.owner + '/' + payload.repository.name) : null,
    sha: payload.after,
    ...
    pusher: payload.user ? payload.user : null,
    owner: (payload.repository && payload.repository.owner) ? payload.repository.owne : null
  };
...
Gith.prototype.simplifyPayload = function( payload ) {
  payload = payload || {};
  if(payload.canon_url && payload.canon_url.match("bitbucket") ){
    return simplifyBitbucketPayload(payload);
  }
    ...
    req.on( "end", function() {
      if ( _.indexOf( gith.ips, req.connection.remoteAddress ) >= 0 ||
        ...
        if ( /^payload=/.test( data ) ) {
          var d = querystring.unescape(data.slice(8));
          if(d.match('"canon_url":+"https://bitbucket.org"')){
            d = d.replace(/\+/g,'');
          }
